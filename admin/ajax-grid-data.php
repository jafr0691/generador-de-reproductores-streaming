...
while( $row=mysqli_fetch_array($query) ) {  // preparing an array
	$nestedData=array();
	if($rol=='admin' or $rol=='vendedor'){
    $usli = mysqli_query($conn, "SELECT * FROM user WHERE id=".$row["user_id"]);
    $usuariolist=mysqli_fetch_assoc($usli);
    $nestedData[] = '<td><span style="text-transform:capitalize">'.$usuariolist["username"].'</span></td>';}
    $nestedData[] = $row["TituloEmisora"];
    $nestedData[] = '<td><span style="text-transform:capitalize">'.$row["Tema"].'</span></td>';
    $nestedData[] = $row["Puerto"];
    $nestedData[] = $row["CPuerto"];
	if($rol=='usuario'){
	    $nestedData[] = '<td><center>
                     <a href="editar.php?id='.$row['ID'].'"  data-toggle="tooltip" title="Editar Reproductor" class="btn btn-sm btn-warning"> <i class="fas fa-edit"></i> </a>
				     </center></td>';
	}else{
	    $nestedData[] = '<td>
	                        <div class="row">
...
$nestedData[] = '<td><center>
                     <a href="https://player.mediapanel.app/?idr='.base64_encode($row['ID']).'"  data-toggle="tooltip" title="Player HTML5" class="btn btn-sm btn-info" target="_blank"> <i class="fas fa-eye"></i> </a>
				     </center></td>';
