	...
	$Seccion=$ArrayGET[0];
    	...
    	if($Seccion!='contacto'){
    		...
    		if(!$IDR || empty($IDR)){
    			...
    			$SVersion=addslashes(htmlentities(strip_tags($_GET['SVersion'])));
    			$Ventana=addslashes(htmlentities(strip_tags($_GET['ventana'])));
    			$Ventana2=addslashes(htmlentities(strip_tags($_GET['ventana2'])));
    			...
    			$Artwork=addslashes(htmlentities(strip_tags($_GET['Artwork'])));
    			$Listeners=addslashes(htmlentities(strip_tags($_GET['Listeners'])));
    			...
    			if($Resultado=$ConsultaDB->query("SELECT * FROM reproductores WHERE ID='$IDR'")){
    				...
    				$SVersion=$Datos['SVersion'];
    				$Tema=$Datos['Tema'];
    				$Ventana=$Datos['ventana'];
    				$Ventana2=$Datos['ventana2'];
    				$Color=$Datos['Color'];
    				if(strlen($Color)==6)
    					$Color='#'.$Datos['Color'];
    				...
    				$Artwork=$Datos['Artwork'];
    				$Listeners=$Datos['Listeners'];
    				$Late=$Datos['Latido'];
    ...
    ?>
  	...
  	<?php
  		switch ($Tema) {
            ...
            break;
            case "tres":
            include('inc/tres.php');
            break;
            case "cuatro":
            include('inc/cuatro.php');
            break;
            case "cinco":
            include('inc/cinco.php');
            break;
            case "seis":
            include('inc/seis.php');
            break;
			default:
